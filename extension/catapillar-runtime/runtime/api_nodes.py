...
try:
    ...
    requests = None  # optional: pip install requests for HTTP flow nodes
...
def node_http_request(ctx):
    ...
    if requests is None:
        msg = "Optional dependency 'requests' not installed. pip install requests for HTTP nodes."
        print("[HTTP] Error:", msg)
        ctx["status_code"] = 0
        ctx["response_body"] = msg
        return {"status_code": 0, "response_body": msg}
