...
def tokenize_line(line: str) -> Optional[Dict]:
    ...
    Returns a dict with: raw_action, raw_args, line_state, optionally emoji_prefix.
...
def tokenize_source(source: str) -> List[Dict]:
    ...
    for lineno, line in enumerate(source.splitlines(), start=1):
        ...
        if "#" in stripped and not stripped.startswith("#"):
            warnings.warn(
                f"Inline comments are not supported in Catapillar v0.1 (ignored) [line {lineno}]",
                CatapillarWarning,
                stacklevel=2
            )
            stripped = stripped.split("#", 1)[0].rstrip()

        token = tokenize_line(stripped)
...
def tokenize_expression(raw_args: List[str]) -> List[str]:
    """Split raw_args into expression tokens. Handles [ ] | ( ) : ： and ``value``."""
